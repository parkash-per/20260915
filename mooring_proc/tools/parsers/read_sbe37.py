...
def _parse_cnv(source_file: Path, row: dict[str, Any]):
    ...
    if "startTime" in proc_header:
        inst_header["startTime"] = proc_header["startTime"]
    ...
    for line in lines:
        if m := re.search(r"# name \d+ = ([^:]+):", line):
            ...
            try:
                ...
                pass
        elif m := re.search(r"#\s*start_time\s*=\s*([^\[]+?)(?:\s*\[|$)", line):
...
def _generate_timestamps(inst_header: dict[str, Any], n_samples: int) -> np.ndarray:
    ...
    start = inst_header.get("startTime", inst_header.get("castDate", 0.0))
    if start == 0.0:
...
def _resolve_input_path(input_path: Any, row: dict[str, Any]) -> Path:
    ...
    raise ValueError(
        "SBE37 input path missing; provide input_path or metadata data_in_path/data_in_file. "
        f"No .cnv/.asc file was found in {base}."
    )
