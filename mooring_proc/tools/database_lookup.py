...
DEFAULT_METADATA_CSV_PATH = Path(
    "/datasets/work/oa-srsalt/work/preqa/SWOT/cal_val/jason_calval/all_mooring_data/reference_mooring_proc_info/satellite_altimetry_moorings_metadata.csv"
)
...
def _coerce_table(source: Any, table_name: str | None = None) -> tuple[pd.DataFrame, Path | None]:
    ...
    # All file-based metadata reads are pinned to the canonical metadata CSV.
    source_path = DEFAULT_METADATA_CSV_PATH
    if not source_path.exists():
        raise FileNotFoundError(f"Metadata CSV was not found: {source_path}")
...
def load_metadata_table(source=None, table_name=None):
