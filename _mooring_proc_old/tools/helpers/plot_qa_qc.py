...
---------
This module plots data series grouped by QC flag values using Plotly. The main entry
point is ``plot_data_by_qc``.

Basic usage:
    fig = plot_data_by_qc(dataset)
    fig = plot_data_by_qc(dataset, variables=["TEMP", "PRES"])
    fig = plot_data_by_qc(
        dataset,
        variables=["TEMP", "PSAL"],
        title="QC plot for temperature and salinity",
        x_start="2024-01-01",
        x_end="2024-01-31",
        y_zoom_to_good=True,
        style="clean",
...
- If ``variables`` is not given, the function uses the default mapping list.
- Each variable is matched to a QC companion using either:
    - the default convention ``<variable>`` + ``<variable>_quality_control``
    - or a custom ``mappings`` list of ``(variable_name, qc_name, y_label)`` tuples.
...
Example of custom mapping:
    ...
    fig = plot_data_by_qc(dataset, mappings=mappings)

Other useful controls:
- ``time_name``: name of the time variable in the dataset (default is ``TIME``).
- ``flags_to_plot``: restrict plotting to selected QC flags, e.g. ``[1]`` or ``{1, 2}``.
- ``legend``: show or hide plot legends.
- ``y_zoom_to_good``: zoom the y-axis to the valid ``QC = 1`` data range.
- ``style``: ``"clean"`` or ``"classic"``.

The function will raise a ``ValueError`` if the requested variable and QC pair are not
